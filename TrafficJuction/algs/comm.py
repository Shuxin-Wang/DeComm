...
class CommNetMLP(nn.Module):
    ...
    def forward(self, x, info={}):
        ...
        agent_mask = agent_mask.to(device=x.device, dtype=x.dtype)

        # Hard Attention - action whether an agent communicates or not
        if self.args.hard_attn:
            comm_action = torch.tensor(info['comm_action']).to(self.args.device)
            comm_action_mask = comm_action.expand(batch_size, n, n).unsqueeze(-1).to(dtype=agent_mask.dtype)
            # action 1 is talk, 0 is silent i.e. act as dead for comm purposes.
            agent_mask *= comm_action_mask.float()
        ...
        for i in range(self.comm_passes):
            ...
            mask = self.comm_mask.to(device=comm.device, dtype=comm.dtype).view(1, n, n)
            ...
            comm_sum = comm_sum.to(dtype=self.C_modules[i].weight.dtype)
