...
from utils.action_utils import select_action, translate_action
...
class GACommNetMLP(nn.Module):
    def __init__(self, args, num_inputs):
        ...
        if self.args.comm_mask_zero:
            self.register_buffer('comm_mask', torch.zeros(self.nagents, self.nagents))
        else:
            self.register_buffer('comm_mask', torch.ones(self.nagents, self.nagents) \
    ...
    def get_agent_mask(self, batch_size, info):
        ...
        agent_mask = agent_mask.float().to(self.args.device)
    ...
    def forward(self, x, info={}):
        ...
        if not self.args.comm_mask_zero:
            if not self.args.comm_action_one:
                ...
                h0 = torch.zeros(2, batch_size * n, self.hid_size * 2, requires_grad=True).to(self.args.device)
                c0 = torch.zeros(2, batch_size * n, self.hid_size * 2, requires_grad=True).to(self.args.device)
            ...
            if 'comm_action' in info:
                comm_action = torch.as_tensor(info['comm_action'], device=self.args.device).view(-1).float()
                if comm_action.numel() == n:    # make sure number of agents is correct
                    total_nodes = hard_attn_output.size(0)
                    repeat = (total_nodes // n) if (total_nodes % n == 0) else 1
                    comm_action_rep = comm_action.repeat(repeat)[:total_nodes]
                    sender_mask_excl = torch.stack(
                        [torch.cat([comm_action_rep[:l], comm_action_rep[l + 1:total_nodes]]) for l in range(total_nodes)],
                        dim=0,
                    )
                    hard_attn_output = hard_attn_output * sender_mask_excl
            ...
            comm = torch.zeros(batch_size, n, self.hid_size).to(self.args.device) if self.args.recurrent else torch.zeros(hidden_state.size()).to(self.args.device)
            comm_density1 = 0
            comm_density2 = 0
    ...
    def init_hidden(self, batch_size):
        ...
        return tuple(( torch.zeros(batch_size * self.nagents, self.hid_size, requires_grad=True).to(self.args.device),
                       torch.zeros(batch_size * self.nagents, self.hid_size, requires_grad=True).to(self.args.device)))
    
    def get_hard_attn_one(self, agent_mask):
        n = self.args.nagents
        adj = torch.ones(n, n).to(self.args.device)
