...
from layers.gnn_layers import GraphAttention
...
class MAGIC(nn.Module):
    ...
    def forward(self, x, info={}):
        ...
        if 'comm_action' in info:
            comm_action = torch.as_tensor(info['comm_action'], device=adj1.device).view(-1).float()
            if comm_action.numel() == n:    # make sure number of agents is correct
                adj1 = adj1 * comm_action.view(1, n).expand(n, n)
        ...
        if 'comm_action' in info:
            comm_action = torch.as_tensor(info['comm_action'], device=adj2.device).view(-1).float()
            if comm_action.numel() == n:
                adj2 = adj2 * comm_action.view(1, n).expand(n, n)
    ...
    def get_agent_mask(self, batch_size, info):
        ...
        if 'alive_mask' in info:
            agent_mask = torch.from_numpy(info['alive_mask']).float().to(self.args.device)
            num_agents_alive = agent_mask.sum()
        else:
            agent_mask = torch.ones(n).to(self.args.device)
            num_agents_alive = n
