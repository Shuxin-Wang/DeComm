...
def translate_action(args, env, action):
    if args.num_actions[0] > 0:
        # environment takes discrete action
        action = [x.squeeze().data.cpu().numpy() for x in action]
        actual = action
        ...
        if args.continuous:
            ...
            for i in range(len(action)):
                low = env.action_space.low[i]
                high = env.action_space.high[i]
                actual[i] = action[i].data.squeeze()[0].cpu().numpy() * (high - low) / (args.naction_heads[i] - 1) + low
            action = [x.squeeze().data[0].cpu().numpy() for x in action]
            return action, actual
