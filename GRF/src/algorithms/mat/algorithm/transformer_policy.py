...
from src.utils.util import get_shape_from_obs_space, get_shape_from_act_space
...
class TransformerPolicy:
    ...
    def __init__(self, args, obs_space, cent_obs_space, act_space, num_agents, device=torch.device("cpu")):
        ...
        if self.algorithm_name in ["commformer", "commformer_dec"]:
            from src.algorithms.mat.algorithm.cformer import CommFormer as MAT
        else:
    ...
    def evaluate_actions(self, cent_obs, obs, rnn_states_actor, rnn_states_critic, actions, masks,
                         available_actions=None, active_masks=None, steps=0, total_step=0, info=None):
        """
